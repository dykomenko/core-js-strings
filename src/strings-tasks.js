...
function removeFirstOccurrences(str, value) {
  ...
  let out = str;
  if (index !== -1) {
    out = str.slice(0, index) + str.slice(index + value.length);
  }
  return out;
}
...
function reverseString(str) {
  ...
  for (let index = 0; index < value.length; index += 1) {
    out += value[value.length - 1 - index];
  }
  return out;
}
...
function isPalindrome(str) {
  ...
  return temp.toLowerCase().split('').reverse().join('') === temp;
}
...
function findLongestWord(sentence) {
  const compareFn = (a, b) => b.length - a.length;
  return sentence.split(' ').sort(compareFn)[0];
}
...
function invertCase(str) {
  const arr = str.split('');
  arr.forEach((sym, index) => {
    arr[index] =
      sym === sym.toLowerCase() ? sym.toUpperCase() : sym.toLowerCase();
  });
  return arr.join('');
}
